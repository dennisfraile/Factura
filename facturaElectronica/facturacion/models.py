...
class Pago(models.Model):
    ...
    formaPago = models.ForeignKey(FormaPago,  ondelete=models.CASCADE)
...
class Identificador(models.Model):
    ...
    tipoDte = models.ForeignKey(TipoDocumento, ondelete=models.CASCADE)
    ...
    tipoOperacion = models.CharField(verbose_name="Tipo de Transmicion", choices=TIPO_TRANSMICION)
...
class SujetoExcluido(models.Model):
    ...
    #Identificador
    identificador = models.ForeignKey(Identificador, ondelete=models.CASCADE)
    
    #Emisor
    emisor = models.ForeignKey(Entidad, ondelete=models.CASCADE)
    ...
    retencionIVAMH = models.CharField(verbose_name="Retencion IVA MH", choices=RETENCION_IVA_MH)
...
class OperacionesSujetoExcluido(models.Model):
    ...
    uniMedida = models.ForeignKey(UnidadMedida,  ondelete=models.CASCADE)
...
class ComprobanteDonacion(models.Model):
    ...
    #Donante
    receptor = models.ForeignKey(Receptor, on_delete=models.CASCADE)
    codDomiciliado = models.CharField(verbose_name="Domicilio Fiscal", choices=DOMICILIO_FISCAL)
...
class OtroDocumentoAsociado(models.Model):
    ...
    codDocAsociado = models.CharField(verbose_name="Documento asociado", choices=OTRO_DOCUMENTO_ASOCIADO)
...
class CuerpoDocumento(models.Model):
    ...
    numItem = models.IntegerField(verbose_name="N° item", min=1, max=2000)
    tipoDonacion = models.CharField(verbose_name="Tipo de donacion", choice=TIPO_DONACION)
...
class Apendice(models.Model):
    ...
    #Facturas Asociadas
    sujetoExcluido = models.ForeignKey(SujetoExcluido, null=True, blank=True, on_delete=models.CASCADE, editable=False, related_name="apendices")
    comprobanteDonacion = models.ForeignKey(ComprobanteDonacion, null=True, blank=True, on_delete=models.CASCADE, editable=False, related_name="apendicesDonacion")
